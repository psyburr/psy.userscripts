...
var factory = {
    ...
    locations : new Array(
        {
            name : 'db9_upload',
            regex : /http(s)?\:\/\/(.*\.)?deepbassnine\.com\/upload\.php.*/i
        },
        {
            name : 'db9_edit',
            regex : /http(s)?\:\/\/(.*\.)?deepbassnine\.com\/torrents\.php\?action=editgroup&groupid=.*/i
        },
        {
            name : 'db9_request',
            regex : /http(s)?\:\/\/(.*\.)?deepbassnine\.com\/requests\.php\?action=new/i
        },
        {
            name : 'db9_request_edit',
            regex : /http(s)?\:\/\/(.*\.)?deepbassnine\.com\/requests\.php\?action=edit&id=.*/i
        },
        {
            name : 'db9_torrent_overview',
            regex : /http(s)?\:\/\/(.*\.)?deepbassnine\.com\/torrents\.php\?id=.*/i
        }
    ),
    ...
    setDefaultFormat : function() {
        ...
        if (default_format !== null && default_format in format_offsets) {
            ...
            switch (this.currentLocation) {
                default:
                    format_select.selectedIndex = format_offsets[defaultDb9Format];
                    break;
            }
        }
    },
            ...
            case "db9_upload":
                var f = function(rawData) {
                    var artist_inputs = document.getElementsByName("artists[]"),
                        ...
                        label_input = document.getElementById("recordlabel"),
                    ...
                    if (data.tags != false) {
                        tags_input.value = (data.tag_string.toLowerCase() == "drum.&.bass" ? "drum.and.bass" : data.tag_string.toLowerCase());
                    } else {
                        tags_input.value = '';
                    }
            ...
            case "db9_request_edit":
                var f = function(rawData) {
                    var artist_inputs = document.getElementsByName("artists[]"),
                        ...
                        label_input = document.getElementsByName("recordlabel")[0],
                        catalog_input = document.getElementsByName("cataloguenumber")[1],
                        tags_input = document.getElementById("tags"),
                        data = yadg.prepareRawResponse(rawData);
